...
def f(str_left, key):
    ...
    s_change_output = s_change(xor_output)          # s盒压缩替代
    res = p_change(s_change_output)                 # p盒置换
    return res
...
def encrypt():
    str=input('请输入你要加密的明文：')
    bin_str = str2bin(str)                                      #数据转为二进制并填充并分组
    bin_key = key2bin(input('请输入你用来加密的密钥：'))             #密钥转为二进制并填充
    print('明文的16进制为', bytes(str, 'UTF-8').hex())
    ...
    for i in tmp:
        ...
        for j in range(15):                                    #先循环15次 因为最后一次不需要不用换位
            f_res = f(str_right, key_lst[j])                   #e盒扩展和子密钥异或
            str_left = xor_change(f_res, str_left)             #右32位与上一轮左32位异或
            str_left, str_right = str_right, str_left          #

        f_res = f(str_right, key_lst[15])                      #第16次
        str_left = xor_change(str_left, f_res)
        fin_str = ip_re_change(str_left + str_right)           #ip的逆置换
        result += fin_str
    last = bin2str(result)
    print('密文为:', last)
    print('密文的16进制为',bytes(last,'UTF-8').hex())
    return last
